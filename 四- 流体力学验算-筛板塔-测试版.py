"""
筛板塔流体力学验算程序（分别计算精馏段和提馏段） - 适配修正版
功能：
1. 塔板压降计算 (干板压降 + 液层压降 + 表面张力压降)
2. 液沫夹带计算
3. 漏液点气速计算
4. 降液管液泛验算
5. 精馏段和提馏段负荷性能图绘制
...
class SieveTowerHydrodynamicCheck:
    """筛板塔流体力学验算器（分别计算精馏段和提馏段）"""

    def __init__(self, plate_arrangement_file='tower_plate_arrangement_corrected.json',
    ...
    def load_data(self, plate_file, diameter_file, properties_file):
        ...
        try:
            ...
            # 提取筛板塔关键参数
            self.extract_sieve_plate_parameters()
    ...
    def extract_sieve_plate_parameters(self):
        """提取筛板塔关键设计参数（塔板结构参数）"""
        # 塔板基本信息
        ...
        self.A_t = area_data['塔板总面积_At_m2']  # 塔板总面积，m²
        self.A_a = area_data['鼓泡区面积_Aa_m2']  # 鼓泡区面积，m²

        # 注意：塔板面积分区中的降液管面积是投影面积
        self.A_f_projected = area_data['总降液管面积_total_Af_m2']  # 总降液管投影面积，m²

        # 筛板布置参数
        sieve_data = self.plate_data['筛板布置']
        self.N_holes = sieve_data['筛孔数']  # 筛孔数
        self.d0 = sieve_data['筛孔直径_m']  # 筛孔直径，m
        self.δ = sieve_data['筛板厚度_m']  # 筛板厚度，m
        self.t = sieve_data['筛孔中心距_m']  # 筛孔中心距，m
        self.φ = sieve_data['开孔率_%'] / 100  # 开孔率（小数）
        self.A0_total = sieve_data['总筛孔面积_m2']  # 总筛孔面积，m²

        # 单个筛孔面积
        self.A0_single = math.pi * (self.d0 / 2) ** 2  # m²
        ...
        # 使用降液管设计中的降液管面积（流通面积）
        downcomer_data = self.plate_data['降液管设计']
        self.A_f = downcomer_data['降液管面积_Af_m2']  # 降液管流通面积，m²
        ...
        print(f"筛孔数: {self.N_holes}")
        print(f"筛孔直径: {self.d0 * 1000:.1f} mm")
        print(f"开孔率: {self.φ * 100:.1f}%")
        print(f"总筛孔面积: {self.A0_total:.4f} m²")
        print(f"堰长: {self.lw:.3f} m, 堰高: {self.hw * 1000:.1f} mm")
        print(f"堰上液层高度: {self.how * 1000:.1f} mm")
        print(f"清液层高度: {self.hL * 1000:.1f} mm")
        print(f"降液管底隙高度: {self.h0 * 1000:.1f} mm")
    ...
    def calculate_dry_plate_resistance_coefficient(self):
        """
        计算干板阻力系数C0
        公式: C0 = 0.670 - 0.115*X1 + 0.514*X2 + 0.228*X1^2 + 0.0682*X1*X2 + 0.441*X2^2
        其中: X1 = d0/δ, X2 = A0/(AT-Af)
        """
        if self.verbose:
            print(f"\n  计算干板阻力系数C0:")

        X1 = self.d0 / self.δ  # 孔径与板厚比
        A_effective = self.A_t - self.A_f_projected  # 使用投影面积计算有效面积
        X2 = self.A0_total / A_effective

        if self.verbose:
            print(f"    计算参数:")
            print(f"      d0 = {self.d0:.4f} m, δ = {self.δ:.4f} m")
            print(f"      X1 = d0/δ = {self.d0:.4f}/{self.δ:.4f} = {X1:.4f}")
            print(f"      A0_total = {self.A0_total:.6f} m²")
            print(
                f"      A_effective = A_t - A_f_projected = {self.A_t:.4f} - {self.A_f_projected:.4f} = {A_effective:.4f} m²")
            print(f"      X2 = A0_total/A_effective = {self.A0_total:.6f}/{A_effective:.4f} = {X2:.4f}")

        self.print_formula("干板阻力系数C0",
                           "C0 = 0.670 - 0.115*X1 + 0.514*X2 + 0.228*X1^2 + 0.0682*X1*X2 + 0.441*X2^2")

        # 计算C0
        C0 = (0.670 - 0.115 * X1 + 0.514 * X2 +
              0.228 * X1 ** 2 + 0.0682 * X1 * X2 + 0.441 * X2 ** 2)

        if self.verbose:
            print(f"    代入计算:")
            print(
                f"      C0 = 0.670 - 0.115*{X1:.4f} + 0.514*{X2:.4f} + 0.228*{X1:.4f}^2 + 0.0682*{X1:.4f}*{X2:.4f} + 0.441*{X2:.4f}^2")
            print(
                f"        = {0.670:.3f} - {0.115 * X1:.4f} + {0.514 * X2:.4f} + {0.228 * X1 ** 2:.4f} + {0.0682 * X1 * X2:.4f} + {0.441 * X2 ** 2:.4f}")
            print(f"        = {C0:.4f}")

        # 孔径大于10mm时的修正系数β=1.15
        if self.d0 > 0.01:  # 10mm = 0.01m
            β = 1.15
            C0_original = C0
            C0 *= β
            if self.verbose:
                print(f"    孔径大于10mm，应用修正系数β={β:.2f}")
                print(f"    C0 = {C0_original:.4f} × {β:.2f} = {C0:.4f}")
    ...
    def calculate_plate_pressure_drop(self, V_m3s, ρ_v, ρ_L, σ, HT, section_name="精馏段"):
        ...
        """
        if self.verbose:
            print(f"\n  计算{section_name}塔板压降:")

        # 1. 计算筛孔气速
        u0 = V_m3s / self.A0_total  # 筛孔气速，m/s
        self.print_calculation("筛孔气速u0", f"V/A0_total = {V_m3s:.6f}/{self.A0_total:.6f}", u0, "m/s")

        # 2. 计算干板阻力系数C0
        C0 = self.calculate_dry_plate_resistance_coefficient()

        # 3. 计算干板压降 hc
        self.print_formula("干板压降hc", "hc = 0.051 × (u0/C0)² × (ρ_v/ρ_L) × [1 - (A0_total/A_a)²]")

        term1 = (u0 / C0) ** 2
        term2 = ρ_v / ρ_L
        term3 = 1 - (self.A0_total / self.A_a) ** 2

        if self.verbose:
            print(f"    计算各项:")
            print(f"      (u0/C0)² = ({u0:.4f}/{C0:.4f})² = {term1:.6f}")
            print(f"      ρ_v/ρ_L = {ρ_v:.4f}/{ρ_L:.1f} = {term2:.6f}")
            print(
                f"      1 - (A0_total/A_a)² = 1 - ({self.A0_total:.6f}/{self.A_a:.4f})² = 1 - {self.A0_total / self.A_a:.4f}² = {term3:.4f}")

        hc = 0.051 * term1 * term2 * term3
        self.print_calculation("干板压降hc", f"0.051 × {term1:.6f} × {term2:.6f} × {term3:.4f}", hc, "m液柱")
        ...
        # 5. 计算表面张力压降 hσ
        self.print_formula("表面张力压降hσ", "hσ = (4σ)/(ρ_L × g × d0)")
        hσ = (4 * σ) / (ρ_L * self.g * self.d0)
        self.print_calculation("表面张力压降hσ",
                               f"(4×{σ:.6f})/({ρ_L:.1f}×9.81×{self.d0:.4f})",
                               hσ, "m液柱")
    ...
    def calculate_leakage_point_velocity(self, L_m3s, ρ_v, ρ_L, σ, section_name="精馏段"):
        """
        计算漏液点气速（筛板塔下限气速）
        ...
        # 计算表面张力压头
        self.print_formula("表面张力压头hσ", "hσ = (4σ)/(ρ_L × g × d0)")
        hσ = (4 * σ) / (ρ_L * self.g * self.d0)
        self.print_calculation("表面张力压头hσ",
                               f"(4×{σ:.6f})/({ρ_L:.1f}×9.81×{self.d0:.4f})",
                               hσ, "m")
        ...
        # 漏液点气速计算公式
        self.print_formula("漏液点气速u_ow", "u_ow = 4.4 × C0 × √[(0.0056 + 0.13×hL - hσ) × ρ_L/ρ_v]")

        term = 0.0056 + 0.13 * hL - hσ
        if term <= 0:
            term = 0.0056
            if self.verbose:
                print(f"    计算项 (0.0056 + 0.13×hL - hσ) = 0.0056 + 0.13×{hL:.4f} - {hσ:.4f} ≤ 0")
                print(f"    取 term = 0.0056")
        else:
            self.print_calculation("计算项", f"0.0056 + 0.13×{hL:.4f} - {hσ:.4f}", term, "")

        u_ow = 4.4 * C0 * math.sqrt(term * ρ_L / ρ_v)
        self.print_calculation("漏液点气速u_ow",
                               f"4.4 × {C0:.4f} × √({term:.6f} × {ρ_L:.1f}/{ρ_v:.4f})",
                               u_ow, "m/s")
    ...
    def calculate_flooding_limit_line(self, ρ_v, ρ_L, σ, HT, section_name="精馏段"):
        ...
        if self.verbose:
            ...
            print(f"        其中: hc = 0.051 × (u0/C0)² × (ρ_v/ρ_L) × [1 - (A0_total/A_a)²]")
            print(f"              hl = ε0 × hL")
            print(f"              ε0 = 0.971 - 0.355×F_a + 0.0757×F_a²")
            print(f"              hσ = (4σ)/(ρ_L × g × d0)")
        ...
        for i, Ls in enumerate(Ls_values):
            ...
            def flooding_equation(Vs):
                ...
                # 干板压降
                hc = 0.051 * (u0 / C0) ** 2 * (ρ_v / ρ_L) * (1 - (self.A0_total / self.A_a) ** 2)
                ...
                # 堰上液层高度
                E = 1.0
                how = 2.84 / 1000 * E * ((Ls * 3600) / self.lw) ** (2 / 3)
                hL = self.hw + how
            ...
            try:
                ...
                if f_min * f_max < 0:
                    ...
                    if self.verbose and i == 0:
                        ...
                        print(f"        求解得: Vs = {Vs_root:.6f} m³/s")
                        # 计算验证
                        u0 = Vs_root / self.A0_total
                        C0 = self.calculate_dry_plate_resistance_coefficient()
                        hc = 0.051 * (u0 / C0) ** 2 * (ρ_v / ρ_L) * (1 - (self.A0_total / self.A_a) ** 2)
                        u_a = Vs_root / self.A_a
                        F_a = u_a * math.sqrt(ρ_v)
                        ε0 = 0.971 - 0.355 * F_a + 0.0757 * F_a ** 2
                        ε0 = max(0.4, min(ε0, 0.7))
                        how = 2.84 / 1000 * 1.0 * ((Ls * 3600) / self.lw) ** (2 / 3)
                        hL = self.hw + how
                        hl = ε0 * hL
                        hσ = (4 * σ) / (ρ_L * self.g * self.d0)
                        hp = hc + hl + hσ
                        hd = 0.153 * (Ls / (self.lw * self.h0)) ** 2 if Ls > 0 else 0
                        Hd = hp + hL + hd
                        print(f"        验证: Hd = {Hd:.4f} m, Hd_max = {Hd_max:.4f} m, 差 = {Hd - Hd_max:.6f}")
                else:
                    # 无根，使用近似值
                    Vs_values.append(Vs_max if f_max < 0 else Vs_min)
                    if self.verbose and i == 0:
    ...
    def calculate_leakage_limit_line(self, ρ_v, ρ_L, σ, section_name="精馏段"):
        ...
        if self.verbose:
            print(f"    方程推导:")
            print(f"      1. 筛孔气速: u0 = Vs / A0_total")
            print(f"      2. 漏液点气速: u_ow = 4.4 × C0 × √[(0.0056 + 0.13×hL - hσ) × ρ_L/ρ_v]")
            print(f"      3. 堰上液层高度: how = 2.84×10⁻³ × E × (L_h/l_w)^(2/3)")
            print(f"      4. 清液层高度: hL = hw + how")
            print(f"      5. 表面张力压头: hσ = (4σ)/(ρ_L × g × d0)")
            print(f"      6. 令 u0 = u_ow，则 Vs = u_ow × A0_total")
        ...
        for i, Ls in enumerate(Ls_values):
            # 计算漏液点气速
            u_ow = self.calculate_leakage_point_velocity(Ls, ρ_v, ρ_L, σ, section_name)

            # 对应的气相流量 Vs = u_ow * A0_total
            self.print_formula("气相流量Vs", "Vs = u_ow × A0_total")
            Vs = u_ow * self.A0_total
            self.print_calculation("Vs", f"{u_ow:.4f} × {self.A0_total:.6f}", Vs, "m³/s")
            ...
            if self.verbose and i == 0:
                ...
                print(f"        气相流量 Vs = {u_ow:.4f} × {self.A0_total:.6f} = {Vs:.6f} m³/s")
    ...
    def plot_load_performance_diagram_for_section(self, Ls_ent, Vs_ent, Ls_flood, Vs_flood,
        ...
        plt.title(f'{section_name}负荷性能图', fontsize=14, fontweight='bold')
        ...
        # 保存图形
        filename = f'sieve_tower_load_performance_{section_name}.png'
    ...
    def save_check_results(self, filename='sieve_tower_hydrodynamic_check_complete.json'):
        """保存完整的验算结果到JSON文件"""
        results = {
            "设计信息": {
                "项目": "筛板塔流体力学验算（精馏段+提馏段）",
                "塔径_m": self.D,
            ...
            "塔板结构参数": {
                "筛孔数": self.N_holes,
                "筛孔直径_m": self.d0,
                "筛孔直径_mm": self.d0 * 1000,
                "开孔率_%": self.φ * 100,
                "筛板厚度_m": self.δ,
                "筛孔中心距_m": self.t,
                "总筛孔面积_m2": self.A0_total,
                "鼓泡区面积_m2": self.A_a,
                "堰长_m": self.lw,
                "堰高_m": self.hw,
                "堰上液层高度_m": self.how,
        ...
        try:
            ...
            print(f"\n✓ 完整验算结果已保存到 '{filename}'")
            print(f"✓ 精馏段负荷性能图已保存为 'sieve_tower_load_performance_精馏段.png'")
            print(f"✓ 提馏段负荷性能图已保存为 'sieve_tower_load_performance_提馏段.png'")
...
def main():
    ...
    try:
        # 创建验算器
        checker = SieveTowerHydrodynamicCheck(
            plate_arrangement_file='tower_plate_arrangement_corrected.json',
            diameter_design_file='tower_diameter_approximate_optimized.json',
            properties_file='tower_properties_results.json'
        )
